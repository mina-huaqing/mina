...
public class RebootPortProcess {
	...
	public RebootPortProcess(String localAddress , MongoOperations mongoOps , CloudNodeIoHandler ioHandler) {
		this.localAddress = localAddress;
		this.mongoOps = mongoOps;
		this.ioHandler = ioHandler;
	}
	
	public void p0x0206(IoSession session, PacketsEntity packets) {
		if (logger.isDebugEnabled()) {
			logger.debug("TAG:0x02XXH,DAS的下行命令包");
		}
		// 解析CMD数据包
		CMDDasPackets cmdParkets = (CMDDasPackets) packets.getValuePackets();
		if (logger.isDebugEnabled()) {

...
public class RTUProcess0713 implements RTUProcess {

	@Override
	public List<Equipment0713> process(RTUPackets rtuPackets) throws RTUProcessException {

		List<Equipment0713> list = new ArrayList<Equipment0713>();
		Equipment0713 data;
		int i = 0;
		while (i < rtuPackets.getValue().length) {
			try {
				data = new Equipment0713();
...
*/				
				...
				data.setCollectTime(Arrays.copyOfRange(rtuPackets.getValue(), i += 1, i += 4));//lz
				i++;
				list.add(data);
